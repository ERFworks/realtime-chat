...
from tests.unit.fakes import FakeProfileRepository, FakeSession, FakeFileStorage, FakeUpload
...
async def test_get_my_profile_returns_profile():
    repo = FakeProfileRepository({1: get_profile()})
    result = await get_my_profile(repo, FakeFileStorage(), user_id=1)
...
async def test_get_my_profile_missing_raises_404():
    with pytest.raises(HTTPException) as exc:
        await get_my_profile(FakeProfileRepository(), FakeFileStorage(),user_id=5)
...
async def test_update_bio_updates_profile():
    profile = get_profile()
    repo = FakeProfileRepository({1: profile})
    result = await update_bio(FakeSession(), repo, FakeFileStorage(),user_id=1, biography="hello")
...
async def test_set_profile_picture_replace_old():
    ...
    result = await set_profile_picture(
        FakeSession(), repo, storage, user_id=1, user_guid=guid, 
...
async def test_set_profile_picture_rejects_wrong_type():
    ...
    with pytest.raises(HTTPException) as exc:
        await set_profile_picture(
            FakeSession(), repo, FakeFileStorage(), user_id=1, user_guid=uuid.uuid4(),
